...
<header class="bg-blue-900 text-white p-4" x-data="{ open: false }">
    <div class="container mx-auto flex justify-between items-center">
        ...
        <nav class="hidden md:flex items-center space-x-4">
            ...
            @auth
                <x-nav-link url="{{route('bookmarks')}}" :active="request()->is(route('bookmarks'))">Saved Jobs</x-nav-link>
                <x-button-link url="/jobs/create" bgColor="yellow-500" textColor="black" hoverClass="yellow-600"
                    icon="edit">Create Job</x-button-link>
                <div class="flex items-center space-x-3">
                    <a href="{{route('dashboard')}}">
                        @if (auth()->user()->avatar)
                            <img src="{{asset('storage/'.auth()->user()->avatar)}}" alt="" class="w-10 h-10 rounded-full">
                        @else
                            <img src="{{asset('storage/avatars/default-avatar.png')}}" alt="" class="w-10 h-10 rounded-full">
                        @endif
                    </a>
                </div>
                <x-log-out-button />
    ...
    <nav x-show="open" @click.away="open = false" id="mobile-menu"
        ...
        @auth
            ...
            <x-button-link url="/jobs/create" bgColor="yellow-500" textColor="black" hoverClass="yellow-600" icon="edit"
                :block="true">Create Job</x-button-link>
            <x-log-out-button />
        @else
